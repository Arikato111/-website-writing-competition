...
if(isset($_POST['regis'])) {
    if($_POST['mem_password'] == $_POST['mem_password2']) {
        if($db->query("SELECT * FROM member WHERE mem_username = '{$_POST['mem_username']}'")->num_rows == 0) {
            ...
            getAlert("สมัครสำเร็จ กรุณาเข้าสู่ระบบเพื่อใช้งาน", 'success');
        } else {
...
<?php FrameStart(); ?>
<h3 class="bg-white rounded shadow-sm p-3 my-3 text-center">สมัครสมาชิก</h3>
<form method="post" class="bg-white rounded shadow-sm p-3 my-3">
    ...
    <div class="text-end">
        <button name="regis" class="btn btn-primary">สมัคร</button>
    </div>
</form>
<?php FrameEnd(); ?>