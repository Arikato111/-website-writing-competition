<?php
if(!isset($_GET['new_id'])) return require('./pages/_error.php');

if(isset($_POST['likeNews'])) {
    $db->query("INSERT INTO `news_like`(`new_like_id`, `new_id`, `mem_id`) 
    VALUES (NULL,'{$_GET['new_id']}','{$_SESSION['usr']}')");
    header("Refresh:0");die;
}
$db->query("UPDATE news SET `new_view`= `new_view`+1 WHERE new_id = " . $_GET['new_id']);
$allNews=$db->query("SELECT * FROM news WHERE new_id = " . $_GET['new_id']);
?>
<title>ประชาสัมพันธ์</title>
<?php FrameStart(); ?>
<div class="text-end my-3">
    <a href="/11pdln/home" class="btn btn-outline-danger">ย้อนกลับ</a>
</div>
<?php $news=fetch($allNews);
...
<?php
require('./com/news/CreateComment.php');
require('./com/news/ShowComment.php');
